...
fit1 <- lm(TransitFrequency ~ PctTransportCosts, data=data)
summary(fit1)
...
fit2 <- lm(TransitFrequency ~ income_thousands, data=data)
summary(fit2)
```

```{r, results=FALSE, message=FALSE}
summary(data$transport_costs)
summary(data$income_thousands) #THE INCOME IS CAPPED AT 250K. This causes issues for regression interpretation. 
